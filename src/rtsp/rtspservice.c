...
//gethostname取得主机名字，在开发板上为空，在电脑上为lucifer(主机名)
	...
	dbg("\n");
	dbg("\thostname is: %s\n", stPrefs.hostname);
	dbg("\trtsp listening port is: %d\n", stPrefs.port);
	dbg("\tInput rtsp://hostIP:%d/test.264 to play this\n",stPrefs.port);
	dbg("\n");
	#endif
...
void AddClient(RTSP_buffer **ppRtspList, int fd)
{
	...
	dbg("Incoming RTSP connection accepted on socket: %d\n",pRtsp->fd);
...
int RTSP_full_msg_rcvd(RTSP_buffer *rtsp, int *hdr_len, int *body_len)
{
	...
	if (rtsp->in_buffer[0] == '$')
	{
		...
		{
			dbg("Interleaved RTP or RTCP packet arrived (len: %hu).\n", bl);
			if (hdr_len)
				*hdr_len = 4;
			if (body_len)
				*body_len = bl;
		...
		{
			/*缓冲区不能完全存放数据*/
			dbg("Non-complete Interleaved RTP or RTCP packet arrived.\n");
		...
		*/
		...
		if (!mb)
		{
			...
			if (!strncmp(&(rtsp->in_buffer[ml]), HDR_CONTENTLENGTH, strlen(HDR_CONTENTLENGTH)))
			{
				...
				{
					dbg("RTSP_full_msg_rcvd(): Invalid ContentLength encountered in message.\n");
					return ERR_GENERIC;
				}
			}
		}
...
int RTSP_valid_response_msg(unsigned short *status, RTSP_buffer * rtsp)
{
    ...
    {
        dbg("Invalid sequence number returned in response.\n");
...
/*/=====================================
	...
	{
		//dbg("Failed to get host eth0 ip\n");
		strcpy(stIfr.ifr_name, "wlan0");
		if(ioctl(pRtsp->fd, SIOCGIFADDR, &stIfr) < 0)
		{
			dbg("Failed to get host eth0 or wlan0 ip\n");
		}
	}
...
int SendDescribeReply(RTSP_buffer * rtsp, char *object, char *descr, char *s8Str)
{
	...
	if (!pMsgBuf)
	{
		dbg("send_describe_reply(): unable to allocate memory\n");
...
int RTSP_describe(RTSP_buffer * pRtsp)
{
	...
	switch (ParseUrl(s8Url, server, &port, object, sizeof(object)))
	{
		case 1: /*请求错误*/
			...
			dbg("url request error");
			return ERR_NOERROR;
			break;

		case -1: /*内部错误*/
			dbg("url error while parsing !\n");
...
int RTSP_setup(RTSP_buffer * pRtsp)
{
	...
	{
		dbg("SETUP request malformed: Transport string is empty\n");
		send_reply(400, 0, pRtsp);       // Bad Request
		dbg("check transport 400 bad request");
		return ERR_NOERROR;
	}
...
//			Transport.rtcp_fd_in = -1;
	...
	{
		dbg("AAAAAAAAAAA Unsupported Transport,%s,%d\n", __FILE__, __LINE__);
...
int RTSP_play(RTSP_buffer * pRtsp)
{
	...
	{
		send_reply(400, 0, pRtsp);// bad request
		dbg("Session Not Found bad request");
		return ERR_NOERROR;
	}
...
int RTSP_teardown(RTSP_buffer * pRtsp)
{
	...
	if (g_s32DoPlay == 0)
	{
		dbg("no user online now resetfifo\n");
		ringreset;
        ...
        case PLAY_STATE:
            {
                switch (method)
                {
                    case RTSP_ID_PLAY:
                        ...
                        dbg("UNSUPPORTED: Play while playing.\n");
        ...
        default:
            {
                ...
                dbg("%s State error: unknown state=%d, method code=%d\n", __FUNCTION__, pRtspSess->cur_state, method);
            }
            break;
    }/* end of current state switch */

	#ifdef RTSP_DEBUG
	dbg("leaving rtsp_state_machine!\n");
	#endif
}
/**************************************************************************************************
**
...
int RtspServer(RTSP_buffer *rtsp)
{
	...
	if (FD_ISSET(rtsp->fd,&rset)) {
		...
		if (rtsp->in_size+n>RTSP_BUFFERSIZE) {
			dbg("RTSP buffer overflow (input RTSP message is most likely invalid).\n");
			send_reply(500, NULL, rtsp);
	...
	if (rtsp->out_size>0) {
		...
		if (n<0)
		{
			dbg("tcp_write error %s %i\n", __FILE__, __LINE__);
			send_reply(500, NULL, rtsp);
			return ERR_GENERIC; //errore interno al server
		}

		#ifdef 	RTSP_DEBUG
		//dbg("OUTPUT_BUFFER length %d\n%s\n", rtsp->out_size, rtsp->out_buffer);
		#endif
		//清空发送缓冲区
		...
		if ((res = RtspServer(pRtsp)) != ERR_NOERROR) {
			if (res==ERR_CONNECTION_CLOSE || res==ERR_GENERIC) {
				...
				if (res==ERR_CONNECTION_CLOSE)
					dbg("fd:%d,RTSP connection closed by client.\n",pRtsp->fd);
				else
					dbg("fd:%d,RTSP connection closed by server.\n",pRtsp->fd);
				...
				if (pRtsp->session_list!=NULL) {
					...
					g_s32DoPlay--;
					if (g_s32DoPlay == 0) {
						dbg("user abort! no user online now resetfifo\n");
						ringreset;
						/* 重新将所有可用的RTP端口号放入到port_pool[MAX_SESSION] 中 */
						RTP_port_pool_init(RTP_DEFAULT_PORT);
					}
					dbg("WARNING! fd:%d RTSP connection truncated before ending operations.\n",pRtsp->fd);
				}
				...
				else
				{
					...
					dbg("dell current fd:%d\n",pRtsp->fd);
					pRtspN->next=pRtsp->next;
					free(pRtsp);
					pRtsp=pRtspN->next;
					dbg("current next fd:%d\n",pRtsp->fd);
				}

				/*适当情况下，释放调度器本身*/
				if (pRtsp==NULL && *conn_count<0) {
					dbg("to stop cchedule_do thread\n");
					stop_schedule=1;
				}
			} else {
				dbg("current fd:%d\n",pRtsp->fd);
	...
	if (s32Fd >= 0) {
		...
		if (!u32FdFound) {
			...
			if (s32ConCnt<MAX_CONNECTION) {
				...
				dbg( "exceed the MAX client, ignore this connecting\n");
				return;
			}
			num_conn++;
			dbg( "%s Connection reached: %d\n", __FUNCTION__, num_conn);
		}
	}
